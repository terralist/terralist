...
func TestDefaultFileServer_ServesMirrorFileWithMirrorResolver(t *testing.T) {
	gin.SetMode(gin.TestMode)

	resolver := newLocalResolver(t)
	key, err := resolver.Store(&storage.StoreInput{
		KeyPrefix: "mirror/registry.terraform.io/hashicorp/null/3.2.4",
		FileName:  "terraform-provider-null_3.2.4_linux_amd64.zip",
		Reader:    bytes.NewReader([]byte("mirrored package")),
		Size:      int64(len("mirrored package")),
	})
	if err != nil {
		t.Fatalf("could not store mirror test file: %v", err)
	}
	...
	if got := w.Body.String(); got != "mirrored package" {
		t.Fatalf("unexpected body: %q", got)
	}
}
