...
func TestProviderDownload(t *testing.T) {
	t.Run("unauthenticated", func(t *testing.T) {
		resp := doUnauthRequest(t, http.MethodGet, apiURL("/v1/providers/hashicorp/null/3.2.4/download/linux/amd64"), nil)
		defer resp.Body.Close()
	...
	t.Run("not found provider", func(t *testing.T) {
		resp := doAuthRequest(t, http.MethodGet, apiURL("/v1/providers/hashicorp/nonexistent/3.2.4/download/linux/amd64"), nil)
		defer resp.Body.Close()
	...
	t.Run("not found version", func(t *testing.T) {
		resp := doAuthRequest(t, http.MethodGet, apiURL("/v1/providers/hashicorp/null/9.9.9/download/linux/amd64"), nil)
		defer resp.Body.Close()
	...
	t.Run("success", func(t *testing.T) {
		resp := doAuthRequest(t, http.MethodGet, apiURL("/v1/providers/hashicorp/null/3.2.4/download/linux/amd64"), nil)
		body := readJSON(t, resp)

		require.Equal(t, http.StatusOK, resp.StatusCode)
		assert.Equal(t, "linux", body["os"])
